...
copyFinder(database, filteredFiles, outputFile)`
  ...
  return (
    <div className="w-full h-full flex items-center justify-center p-0 md:p-2">
      ...
      <div className="w-[90vw] md:max-w-3xl h-full max-h-[28rem] flex flex-col rounded-xl border border-zinc-800 bg-zinc-950 shadow-2xl overflow-hidden">

        <div className="flex items-center justify-between border-b border-zinc-800 bg-zinc-900/50 px-3 py-3 md:px-4 shrink-0">
             <div className="flex items-center gap-2 text-zinc-400">
               <FileIcon className="h-3 w-3 md:h-4 md:w-4 text-blue-400" />
               <span className="text-[10px] md:text-xs font-mono">copyFinder.py</span>
             </div>
        ...
        <div 
          className="flex-1 overflow-auto bg-zinc-950/80 p-3 md:p-4 cursor-text scrollbar-thin scrollbar-thumb-zinc-700 scrollbar-track-transparent"
          onPointerDown={(e) => e.stopPropagation()}
        >
           <div className="text-zinc-300 text-[10px] sm:text-xs md:text-sm font-mono leading-relaxed whitespace-pre font-ligatures-none">
             {pythonCode}
           </div>
        </div>
